...
public class ContextServiceDefinitionNode extends DeploymentDescriptorNode<ContextServiceDefinitionDescriptor> {
    public final static XMLElement tag = new XMLElement(ConcurrencyConstants.CONTEXT_SERVICE);
    ...
    protected java.util.Map getDispatchTable() {
        ...
        table.put(ConcurrencyConstants.CONTEXT_SERVICE_NAME, "setName");
        table.put(ConcurrencyConstants.CONTEXT_SERVICE_PROPAGATED, "addPropagated");
        table.put(ConcurrencyConstants.CONTEXT_SERVICE_CLEARED, "addCleared");
        table.put(ConcurrencyConstants.CONTEXT_SERVICE_UNCHANGED, "addUnchanged");
        return table;
    }
    ...
    public org.w3c.dom.Node writeDescriptor(org.w3c.dom.Node parent, String nodeName,
        ...
        appendTextChild(node, ConcurrencyConstants.CONTEXT_SERVICE_NAME, contextServiceDefinitionDescriptor.getName());
        for(String s : contextServiceDefinitionDescriptor.getCleared()) {
            appendTextChild(node, ConcurrencyConstants.CONTEXT_SERVICE_CLEARED, s);
        }

        for(String s : contextServiceDefinitionDescriptor.getPropagated()) {
            appendTextChild(node, ConcurrencyConstants.CONTEXT_SERVICE_PROPAGATED, s);
        }

        for(String s : contextServiceDefinitionDescriptor.getUnchanged()) {
            appendTextChild(node, ConcurrencyConstants.CONTEXT_SERVICE_UNCHANGED, s);
        }
